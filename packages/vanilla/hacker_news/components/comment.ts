...
function clickToggle(this: Element) {
...
export function setupCommentToggle() {
  document.querySelectorAll("#toggle-click").forEach((el) => {
    el.removeEventListener("click", clickToggle);
  ...
  return `<li class="comment">
      ...
      ${
        ...
        <div id="toggle" data-open='false'>
          ...
          <ul id="toggle-ui" class="comment-children">${comment.comments.map((comment) => Comment({ comment })).join(" ")}</ul>
        </div>`
          : ""
      }
    </li>`;
